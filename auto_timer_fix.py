...
if 'android_alarm_manager_plus' not in code:
  ...
  DateTime nextHour = DateTime(now.year, now.month, now.day, now.hour).add(const Duration(hours: 1));
...
if 'AlarmService' not in mani_code:
    ...
    """
    mani_code = mani_code.replace('</application>', services + '\n    </application>')
    with open(manifest, 'w', encoding='utf-8') as f:
        f.write(mani_code)

print("=== ПАТЧ КОДУ ЗАВЕРШЕНО ===")
